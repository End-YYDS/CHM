...
use chm_grpc::{
    ldap::{
        ldap_service_client::LdapServiceClient, AddWebRoleRequest, AuthRequest, Empty,
        GroupDetailResponse, GroupIdRequest, GroupNameResponse, GroupRequest,
        ModifyGroupNameRequest, ModifyUserRequest, ToggleUserStatusRequest, UserDetailResponse,
        UserGroupRequest, UserIdRequest, UserRequest, WebRoleDetailResponse,
    },
...
impl ClientLdap {
    ...
    pub async fn add_web_role(
        ...
        role_name: String,
        color: i32,
        color_number: Option<String>,
        permission: i32,
    ) -> ConResult<bool> {
        let mut client = self.client.clone();
        let req = AddWebRoleRequest { role_name, color, color_number, permission };
        let resp = client
            .add_web_role(req)
            .await
            .inspect_err(|e| tracing::error!(?e))?

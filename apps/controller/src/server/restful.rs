...
use chm_grpc::{
    ...
    restful::{restful_service_server::RestfulService, Color, *},
    tonic,
...
impl RestfulService for ControllerRestfulServer {
    ...
    async fn get_roles(
        ...
        let ldap = self
            .grpc_clients
            .try_ldap()
            .map_err(|e| Status::internal(format!("LDAP client init failed: {}", e)))?;
        let r = ldap
            .list_web_roles()
            .await
    ...
    async fn create_role(
        ...
        let (color_i32, color_number) = match req.color.and_then(|c| c.kind) {
            Some(color::Kind::Predefined(v)) => (v, None),
            Some(color::Kind::Custom(s)) => (0, Some(s)),
            None => (0, None),
        };
        let permission = req.permissions as i32;
        ldap.add_web_role(role_name, color_i32, color_number, permission).await.map_err(|e| {

...
use sysinfo::{
    CpuExt, CpuRefreshKind, DiskExt, ProcessExt, ProcessRefreshKind, RefreshKind, System, SystemExt,
};
...
pub struct HostdExtension {
    #[serde(default = "HostdExtension::default_socket_path")]
    pub socket_path: String,
    #[serde(default = "HostdExtension::default_max_concurrency")]
    pub max_concurrency: usize,
    #[serde(default = "HostdExtension::default_command_timeout")]
    pub command_timeout_secs: u64,
}

impl HostdExtension {
    fn default_socket_path() -> String {
        "/tmp/agent_hostd.sock".to_string()
    }
...
impl Default for HostdExtension {
    fn default() -> Self {
        Self {
...
declare_config!(extend = crate::HostdExtension);
declare_config_bus!();
...
pub fn socket_path() -> String {
    GlobalConfig::with(|cfg| cfg.extend.socket_path.clone())
}

pub fn max_concurrency_limit() -> usize {
    let value = GlobalConfig::with(|cfg| cfg.extend.max_concurrency);
    if value == 0 {
        DEFAULT_MAX_CONCURRENCY
    } else {
        value
...
pub fn command_timeout_secs() -> u64 {
    let secs = GlobalConfig::with(|cfg| cfg.extend.command_timeout_secs);
    if secs == 0 {
        DEFAULT_COMMAND_TIMEOUT_SECS
    } else {
        secs
...
pub fn execute_sysinfo(command: &str) -> Result<String, String> {
    ...
    match keyword {
        ...
        "memory_status" => {
            sys.refresh_specifics(RefreshKind::new().with_memory());
            let total = sys.total_memory() as f32;
            let used = sys.used_memory() as f32;
            if total == 0.0 {
                Err("memory information unavailable".to_string())
            } else {
                Ok(format!("{:.2}", used / total * 100.0))
            }
        }
        "disk_status" => {
            sys.refresh_specifics(RefreshKind::new().with_disks_list().with_disks());
            let total: u64 = sys.disks().iter().map(|d| d.total_space()).sum();
            let available: u64 = sys.disks().iter().map(|d| d.available_space()).sum();
            if total == 0 {
                Err("disk information unavailable".to_string())
            } else {
                let used = total - available;
                Ok(format!("{:.2}", used as f32 / total as f32 * 100.0))
            }
        }
...
fn build_network_interface_dto(interface: &NetworkInterface) -> NetworkInterfaceDto {
    ...
    let mac = interface.mac.map(|mac| mac.to_string()).unwrap_or_else(String::new);
...
fn collect_dns_info() -> Result<DnsInfoDto, String> {
    ...
    let primary = nameservers.get(0).cloned().unwrap_or_default();
...
fn collect_fallback_processes(sys: &mut System) -> Result<ProcessInfoDto, String> {
    ...
    let entries = names
        .into_iter()
        .map(|name| ProcessEntryDto {
            name,
            status: ProcessStatusDto { status: true, boot: false },
        })
        .collect::<Vec<_>>();
...
fn extract_policy_from_nft(line: &str) -> Option<String> {
    if let Some(start) = line.find("policy") {
        ...
        let token =
            tail.split(|c: char| c.is_whitespace() || c == ';').filter(|s| !s.is_empty()).next()?;
...
fn extract_address(text: &str, key: &str) -> Option<String> {
    if let Some(idx) = text.find(key) {
        ...
        let token =
            rest.split(|c: char| c.is_whitespace() || c == '"').filter(|s| !s.is_empty()).next()?;
...
fn parse_iptables_chain_header(line: &str) -> Option<(String, String)> {
    ...
    if let Some(start) = line.find("(policy ") {
        ...
        let policy = policy_token.trim_end_matches(|c| c == ')' || c == ',').to_uppercase();
        Some((name, policy))
    } else {
...
fn serialize_cron_jobs() -> Result<String, io::Error> {
    ...
    serde_json::to_string(&CronJobsDto { jobs, length })

...
pub struct AgentExtension {
    #[serde(default = "AgentExtension::default_hostd_socket")]
    pub hostd_socket: String,
...
impl AgentExtension {
    fn default_hostd_socket() -> String {
...
impl Default for AgentExtension {
    fn default() -> Self {
        Self {
            hostd_socket: Self::default_hostd_socket(),
...
pub fn hostd_socket_path() -> String {
    crate::globals::GlobalConfig::with(|cfg| cfg.extend.hostd_socket.clone())
}
...
static FAMILY_COMMANDS: FamilyCommands =
    FamilyCommands { crontab: "crontab", ip: "ip", iptables_candidates: &IPTABLES_STANDARD };
...
pub fn send_to_hostd(real_command: &RealCommand) -> io::Result<String> {
    let socket_path = hostd_socket_path();
...
pub fn make_sysinfo_command(keyword: &str) -> RealCommand {
    RealCommand { command: keyword.to_string(), is_get: true, response_kind: ResponseKind::Raw }
}

pub fn make_sysinfo_command_with_argument(keyword: &str, argument_json: &str) -> RealCommand {
    let encoded = general_purpose::STANDARD.encode(argument_json.as_bytes());
    let command =
...
pub fn execute_host_body(body: &str) -> Result<HostCommandOutput, String> {
    ...
    let real = RealCommand { command: payload, is_get: false, response_kind: ResponseKind::Raw };
...
fn encode_script(script: &str) -> String {
    ...
    let encoded = general_purpose::STANDARD.encode(script_owned.as_bytes());

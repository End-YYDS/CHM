...
//             post, resource, Data, HttpResponse, Json, Responder, Sender,
// ServiceConfig,         };
...
// async move {                         if envelope.code.as_str() !=
// otp_code.as_str() {                             return
// HttpResponse::Unauthorized().json($crate::ApiResponse {
// message: "OTP 驗證失敗".into(),                                 ok:
// false,                             });
//                         }
//                         let data_json = Json(envelope.data);
//                         if let ControlFlow::Break(resp) = handler(&req,
// data_json).await {                             return resp;
//                         }
//                         if let Err(e) =
// tokio::fs::write(marker_path.get_ref(), b"done").await {
// return HttpResponse::InternalServerError().json($crate::ApiResponse {
//                                 message: format!("寫入 marker 檔案失敗:
// {e}"),                                 ok:      false,
//                             });
//                         }
...
macro_rules! declare_init_route {
    ($handler:expr, data = $data:ty) => {
        $crate::declare_init_route!($handler, data = $data, extras = ());
    };
            ...
            move |cfg: &mut ServiceConfig| {
                cfg.service(resource("/init").route(post().to({
                    ...
                    move |req: HttpRequest,
                        ...
                        let data_json = Json(envelope.data);
                        if let ControlFlow::Break(resp) = handler(&req, data_json $(, $extra_pat.clone())*).await {
                            return resp;
                        }
                        ...
                        let _ = shutdown_tx.send(()).await;
                        HttpResponse::Ok().json($crate::ApiResponse {
                            message: "初始化完成，web服務器將關閉".into(),
                            ok:      true,
                        })
                    }
